...
if not tarballLocation:
    ...
    if not os.path.exists(destDir):
        print("Creating directory %s for git repo %s" %
    ...
    if os.path.exists('.git'):
        print("Resetting previously existing git repo at %s for receiving git repo %s" % (
            destDir, git_repo[pkgName]))
    ...
    print("Fetching git [REP '%s' BRANCH '%s'] -> %s" %
...
if pkgName == "spice-vd-agent":
    ...
    os.system(
        "sed -i '/(PCIACCESS_CFLAGS)/ s/$/  -fno-strict-aliasing/g' Makefile.am")
