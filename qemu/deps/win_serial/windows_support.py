    ...
    def read(self, n):
        while True:  # emulate blocking IO
            ...
            try:
                ...
                err, b = win32file.ReadFile(
                    self._hfile, 4096, self._read_ovrlpd)
                nr = win32file.GetOverlappedResult(
                    self._hfile, self._read_ovrlpd, True)
                if nr > 0:
        ...
        # Never Reached
        raise Exception(
            "Error in WinBufferedReadFile - should never be reached")
