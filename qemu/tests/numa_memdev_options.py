...
def get_host_numa_node():
    ...
    for i in host_numa:
        node_size = re.findall(r"node %d size: \d+ \w" %
                               i, numa_info)[0].split()[-2]
        if node_size != '0':
...
def check_memory_in_procfs(test, params, vm):
    ...
    for mem_dev in params['mem_devs'].split():
        ...
        smaps = process.system_output(
            r"grep -B1 -E '^Size:\s+%d' /proc/%d/smaps"
            % (mem_size, qemu_pid)
        )
...
def run(test, params, env):
    ...
    if params.get('set_node_hugepage') == 'yes':
        ...
        normalize_total_hg1 = int(
            normalize_data_size(params['size_mem0'], 'K'))
