...
def run(test, params, env):
    ...
    def check_result(qmp_o, expect_o=None):
        ...
        logging.info(
            "Actual result that get from qmp_cmd/post_cmd is %s", qmp_o)
    ...
    if result_check == "equal" or result_check == "contain":
        ...
        if qmp_cmd == "query-name":
            ...
            qemu_version = callback["host_cmd"](
                qemu_version_cmd).replace('.%s' % host_arch, '')
            ...
            expect_o = [{'service': str(vnc_port)}, {'enabled': True}, {
                'host': '0.0.0.0'}]
        check_result(qmp_o, expect_o)
    elif result_check.startswith("post_"):
        logging.info(
            "Verify post qmp command '%s' works as designed.", post_cmd)
