    ...
    try:
        ...
        for t in ["in", "out"]:
            ...
            for x in range(int(nums_cpu)):
                process.run(
                    "mkfifo {}trace-path-cpu{}.{}".format(v_path, x, t))
        ...
        for x in range(int(nums_cpu)):
            ...
            params['chardev_path_vs{}'.format(
                x)] = "{}trace-path-cpu{}".format(v_path, x)
        ...
        for index, proc in enumerate(procs):
            if proc.get_output():
                test.fail(
                    "cpu %s still have output after disabled in host" % index)
            proc.close()
    finally:
