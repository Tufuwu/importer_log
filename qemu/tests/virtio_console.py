...
def run(test, params, env):
    ...
    def test_loopback():
        ...
        for param in test_params.split(';'):
            if not param:
                continue
            error_context.context("test_loopback: params %s" %
                                  param, logging.info)
            # Prepare
            param = param.split(':')
            idx_serialport = 0
    ...
    def test_interrupted_transfer():
        ...
        def _replug_loop():
            ...
            def _port_plug(device, property):
                ...
                if not ver_out:
                    test.error("Error occured when plug port %s." %
                               property['name'])
                time.sleep(intr_time)

            def _pci_unplug(bus):
                device = vm.devices.get_by_params(
                    {"id": str(bus).split('.')[0]})[0]
                if not device:
    ...
    def _tmigrate(use_serialport, no_ports, no_migrations, blocklen, offline):
        ...
        if use_serialport:
            vm, guest_worker = virtio_test.get_vm_with_worker(
                no_serialports=no_ports)
            ports = virtio_test.get_virtio_ports(vm)[1]
        else:
            vm, guest_worker = virtio_test.get_vm_with_worker(
                no_consoles=no_ports)
            ports = virtio_test.get_virtio_ports(vm)[0]
    ...
    def test_hotplug():
        ...
        vm = virtio_test.get_vm_with_ports(
            0, 2, spread=0, quiet=True, strict=True)
        ...
        (result, ver_out) = vm.devices.simple_unplug(portdev, vm.monitor)
        portdev_ = vm.devices.get_by_params(
            {"name": consoles[1][0].qemu_id})[0]
        ...
        if re.search(qemu_version_pattern, output_list[0]):
            ...
            params["extra_params"] = (params["extra_params"]
                                      % (get_virtio_serial_name(),
                                         max_ports_invalid[1]))
            exp_error_message = (
                params['virtio_console_params'] % max_ports_valid[1])
