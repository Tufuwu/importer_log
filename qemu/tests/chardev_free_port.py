...
def run(test, params, env):
    ...
    for vm_ind, vm in enumerate(vms):
        ...
        params['extra_params_%s' % vm] = params.get(
    ...
    for char_ind, chardevs in enumerate(chardev_infos):
        ...
        for chardev in chardevs:
            if chardev['label'] == chardev_params['id']:
                tmp_pnum = int(chardev['filename'].split(
                    ':')[-1].split(',')[0])
                error_context.context("Get port %d for vm%d from monitor" % (
                    tmp_pnum, char_ind), logging.info)
                break
        if char_ind == 0:
            error_context.context("The expect port for vm%d is %d" % (
                char_ind, _port), logging.info)
            if tmp_pnum == _port:
                in_chardev = True
        else:
            error_context.context("The expect port for vm%d is in [%d, %d]" % (
