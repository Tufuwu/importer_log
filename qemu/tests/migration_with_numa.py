...
def run(test, params, env):
    ...
    if os_type == 'linux':
        error_context.context(
            "Check the numa memory size in guest", logging.info)
        ...
        guest_nodes_size = get_nodes_size(
            size_type='MemTotal', session=session)
        ...
        for nodenr, node in enumerate(params.objects('guest_numa_nodes')):
            ...
            if mdev:
                mdev = mdev.split('-')[1]
                size = float(normalize_data_size(
                    params.get("size_mem_%s" % mdev)))
    ...
    error_context.context(
        "Check the numa memory policy in dest host", logging.info)
    qemu_pid = vm.get_pid()
    for mem_dev in mem_devs:
