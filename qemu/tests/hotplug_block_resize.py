...
def run(test, params, env):
    ...
    def _block_resize(dev):
        ...
        resize_size = int(float(normalize_data_size(re.search(
            r'(\d+\.?(\d+)?\w)', params['resize_size']).group(1), "B")))
        size = str(
    ...
    data_image_size = int(float(normalize_data_size(
        data_image_params.get("image_size"), "B")))
    ...
    try:
        if is_windows:
            if not utils_disk.update_windows_disk_attributes(session, plug[:]):
                test.fail(
                    'Failed to clear readonly and online on %s.' % plug[:])
        mount_point = utils_disk.configure_empty_disk(
            session, plug[0], block_size + 'B', os_type)[0]
