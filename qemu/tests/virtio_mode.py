...
def run(test, params, env):
    ...
    def verify_virtio_mode_qtree():
        ...
        if (disable_modern != params["virtio_dev_disable_modern"] or
                disable_legacy != params["virtio_dev_disable_legacy"]):
            test.fail("virtio mode in qtree is not correct, details are %s %s"
    ...
    def verify_virtio_mode_guest_win(session, virtio_mode):
        ...
        if device_id > 1040:
            ...
            guest_mode = win_memory_range(
                session, devcon_folder, hwid, virtio_mode)
