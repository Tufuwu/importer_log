...
def run(test, params, env):
    ...
    def convert_cpu_topology_to_ids(socketid=None, dieid=None, coreid=None,
        ...
        def _get_boundary(value, max_value, weight):
            ...
            max_boundary = int(
                value if value is not None else (max_value - 1)) * weight
    ...
    def numa_cpu_cli():
        ...
        error_context.context(
            "Get the expected cpus in qemu command line", logging.info)
    ...
    if qemu_preconfig:
        ...
        numa_cpu_setted.extend(
            [[]] * (len(numa_cpu_cli) - len(numa_cpu_setted)))
    ...
    numa_cpu_monitor = [sorted(list(item[1]))
                        for item in vm.monitor.info_numa()]
