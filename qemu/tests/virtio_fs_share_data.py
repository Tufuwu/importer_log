...
def run(test, params, env):
    ...
    def get_viofs_exe(session):
        ...
        try:
            get_drive_letter = getattr(
                virtio_win, "drive_letter_%s" % media_type)
            get_product_dirname = getattr(virtio_win,
                                          "product_dirname_%s" % media_type)
            get_arch_dirname = getattr(
                virtio_win, "arch_dirname_%s" % media_type)
    ...
    if cmd_xfstest:
        ...
        ori_tmpfs_size = process.run(
            cmd_get_tmpfs, shell=True).stdout_text.replace("\n", "")
    ...
    for fs in params.objects("filesystems"):
        ...
        if os_type == "linux":
            ...
            error_context.context(
                "Start virtiofs service in guest.", logging.info)
            ...
            if viofs_log_file_cmd:
                error_context.context(
                    "Check if LOG file is created.", logging.info)
        ...
        try:
            ...
            if cmd_pjdfstest:
                error_context.context("Run pjdfstest on %s." %
                                      fs_dest, logging.info)
                host_path = os.path.join(
                    data_dir.get_deps_dir('pjdfstest'), pjdfstest_pkg)
                scp_to_remote(host_addr, port, username,
                              password, host_path, fs_dest)
            ...
            if os_type == "linux":
                utils_disk.umount(fs_target, fs_dest,
