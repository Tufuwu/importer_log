...
def run(test, params, env):
    ...
    def irq_check(session, device_name, devcon_folder, timeout):
        ...
        for hwid in hwids:
            ...
            irq_list = re.findall(
                r':\s+(\d+)', session.cmd_output(get_irq_cmd), re.M)
            if not irq_list:
    ...
    def set_msi_fguest(enable=True):
        ...
        """
        hwid = win_dev.get_hwids(session, device_name,
                                 devcon_folder, timeout)[0]
    ...
    devcon_folder = utils_misc.set_winutils_letter(
        session, params["devcon_folder"])
