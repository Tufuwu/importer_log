...
class QemuGuestAgentBasicCheck(QemuGuestAgentTest):
    ...
    def gagent_check_install_uninstall(self, test, params, env):
        ...
        logging.info(
            "Repeat install/uninstall qemu-ga pkg for %s times", repeats)
    ...
    def gagent_check_stop_start(self, test, params, env):
        ...
        repeats = int(params.get("repeat_times", 1))
        logging.info(
            "Repeat stop/restart qemu-ga service for %s times", repeats)
    ...
    def gagent_check_fsinfo(self, test, params, env):
        ...
        for fs in fs_info_qga:
            ...
            if mount_pt != 'System Reserved':
                ...
                logging.info(
                    "'%s' disk usage statistic is not supported", mount_pt)

            error_context.context("Check file system type of '%s' mount point."
                                  % mount_pt, logging.info)
            fs_type_qga = fs["type"]
            cmd_get_disk = params["cmd_get_disk"] % mount_pt.replace(
                "/", r"\/")
            if params["os_type"] == "windows":
            ...
            if fs_type_qga != fs_type_guest:
                ...
                logging.info(
                    "File system type is %s which is expected.", fs_type_qga)
            ...
            if disk_name_qga != disk_name_guest:
                ...
                logging.info(
                    "Disk name is %s which is expected.", disk_name_qga)
            ...
            if not re.findall(serial_num, serial_qga):
                ...
                logging.info(
                    "Serial number is %s which is expected.", serial_qga)
    ...
    def gagent_check_os_info(self, test, params, env):
        ...
        if os_type == "windows":
            ...
            if "32-bit" in machine_type_guest:
                ...
                test.error(
                    "Only support x86 and x86_64 in this auto test now.")
...
class QemuGuestAgentBasicCheckWin(QemuGuestAgentBasicCheck):
    ...
    def _check_serial_driver(self, test, params, env):
        ...
        def _chk_cert(session, cert_path):
            ...
            if re.search("Incomplete certificate chain", out, re.I):
                logging.warning(
                    "Incomplete certificate chain! Details:\n%s", out)
    ...
    def setup(self, test, params, env):
        ...
        if self.start_vm == "yes":
            ...
            args = [params.get("gagent_serial_type"),
                    params.get("gagent_name")]
            self.gagent_create(params, self.vm, *args)
        ...
        try:
            ...
            try:
                ...
                if re.search("fsfreeze is limited up to 10 seconds",
                             str(detail)):
                    logging.info(
                        "FS is thaw as it's limited up to 10 seconds.")
                else:
