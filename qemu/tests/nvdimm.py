...
class NvdimmTest(object):
    ...
    def run_guest_cmd(self, cmd, check_status=True, timeout=240):
        ...
        if check_status and status != 0:
            self.test.fail(
                "Execute command '%s' failed, output: %s" % (cmd, output))
        return output.strip()
    ...
    def verify_nvdimm(self, vm, mems):
        ...
        if not dimms_expect.issubset(dimms_monitor):
            ...
            self.test.fail("%s dimms are invisible in monitor" %
                           invisible_dimms)
...
def run(test, params, env):
    ...
    try:
        ...
        error_context.context(
            "Verify nvdimm in monitor and guest", logging.info)
        ...
        error_context.context(
            "Compare current md5 to original md5", logging.info)
        if new_md5 != orig_md5:
            test.fail("'%s' changed. The original md5 is '%s', current md5 is '%s'"
                      % (nv_file, orig_md5, new_md5))
        nvdimm_test.umount_nvdimm()
        error_context.context(
            "Check if error and calltrace in guest", logging.info)
