...
def get_keycode_cfg(filename):
    ...
    """
    keycode_cfg_path = os.path.join(
        data_dir.get_deps_dir("key_keycode"), filename)
...
def key_tap_test(test, params, vm):
    ...
    def key_check(key):
        ...
        for k, v in key_event_lst[:-key_num]:
            if v != 'KEYDOWN':
                test.fail(
                    "Received key {0} event type {1} was not KEYDOWN").format(k, v)
        ...
        for k, v in key_event_lst[-key_num:]:
            if v != 'KEYUP':
                test.fail(
                    "Received key {0} event type {1} was not KEYUP").format(k, v)
            key_up_lst.append(k)
