...
def run(test, params, env):
    ...
    def run_sub_test(test_name):
        ...
        error_context.context("Running sub test '%s'." %
                              test_name, logging.info)
    ...
    def plug_block_devices(action, plug_devices):
        ...
        for dev in plug_devices:
            ...
            if ret[1] is False:
                test.fail("Failed to %s device '%s', %s." %
                          (action, dev, ret[0]))
    ...
    def format_disk_win():
        ...
        error_context.context("Format disk %s in windows." %
                              new_disk, logging.info)
        session = vm.wait_for_login(timeout=timeout)
    ...
    def get_disk_size(did):
        ...
        if windows:
            ...
            size = re.search(p % did, disk_info, re.I |
                             re.M).groupdict()['size'].strip()
        else:
