...
def run(test, params, env):
    ...
    def run_stressapptest(timeout):
        ...
        try:
            ...
            sub_session.cmd('{} {}'.format(
                stress_bin_path, stress_options), timeout)
        finally:
    ...
    def run_stress_background(timeout):
        ...
        thread = _StressThread(
            stress_maps[stress_name], exit_event, (timeout, ))
        thread.start()
        return thread
    ...
    def ping_pong_migration(repeat_times):
        ...
        for i in range(repeat_times):
            ...
            if do_migration_background:
                ...
                migration_thread = utils_misc.InterruptedThread(vm.migrate,
                                                                args, kwargs)
    ...
    do_migration_background = params.get(
        'do_migration_background', 'no') == 'yes'
    ...
    error_context.context('Boot guest %s on src host.' %
                          src_desc, logging.info)
    ...
    if params.get('run_stress_before_migration', 'no') == 'yes':
        if do_stress_background:
            ...
            if not utils_misc.wait_for(lambda: (
                    stress_thread.exit_event.is_set() or bg_stress_is_alive(
                    session, stress_name)), 120, step=3):
    ...
    error_context.context('Boot guest %s on dst host.' %
                          dst_desc, logging.info)
