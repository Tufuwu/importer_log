...
def run(test, params, env):
    ...
    def delete_scsi_device(scsi_addr):
        ...
        session.cmd(
            'echo 1 > /sys/class/scsi_device/%s/device/delete' % scsi_addr)
    ...
    def get_scsi_addr_by_product(product_name):
        ...
        addr_info = session.cmd(
            "lsscsi | grep %s | awk '{print $1}'" % product_name)
